...
//        методе жизненного цикла onCreate (...) .
    ...
    protected void initLayout(View view, Bundle savedInstanceState) {
        ...
        String getHum = prefsHelper.getSharedPreferences(Constants.HUMIDITY);
        if (!getHum.equals("")) {

            ((TextView) getBaseActivity().findViewById(R.id.tv_humidity)).setText(prefsHelper.getSharedPreferences(Constants.HUMIDITY));
            ((TextView) getBaseActivity().findViewById(R.id.tv_pressure)).setText(prefsHelper.getSharedPreferences(Constants.PRESSURE));
            ((TextView) getBaseActivity().findViewById(R.id.bigTemp)).setText(prefsHelper.getSharedPreferences(Constants.TEMPERATURE));
            Toast.makeText(getBaseActivity(), "Значение загружены", Toast.LENGTH_LONG).show();
        } else {
            prefsHelper.saveSharedPreferences(Constants.HUMIDITY, "30%");
            prefsHelper.saveSharedPreferences(Constants.TEMPERATURE, "25C");
            prefsHelper.saveSharedPreferences(Constants.PRESSURE, "750mmHg");
            Toast.makeText(getBaseActivity(), "Значение проинициализированы", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void onPause() {
        super.onPause();
//        prefsHelper.saveSharedPreferences(Constants.HUMIDITY,((TextView)getBaseActivity().findViewById(R.id.tv_humidity)).getText().toString());
//        prefsHelper.saveSharedPreferences(Constants.PRESSURE,((TextView)getBaseActivity().findViewById(R.id.tv_pressure)).getText().toString());
//        prefsHelper.saveSharedPreferences(Constants.TEMPERATURE,((TextView)getBaseActivity().findViewById(R.id.bigTemp)).getText().toString());
    }
}
